...
static const double MAG_C0[3] = {-0.002136485053939582, -0.000749655052795221, -0.005386127855323933};
static const double MAG_C1[3] = { 0.2516605407371642,   0.6775232436837668,    2.494026599312351   };
static const double MAG_C2[3] = { 8.353717279216625,   -3.577719514958484,     0.3144679030132573  };
static const double MAG_C3[3] = {-27.66873308576866,    14.26473078096533,    -13.64921318813922   };
static const double MAG_C4[3] = { 52.17613981234068,   -27.94360607168351,     12.94416944238394   };
static const double MAG_C5[3] = {-50.76852536473588,    29.04658282127291,      4.23415299384598   };
static const double MAG_C6[3] = { 18.65570506591883,   -11.48977351997711,     -5.601961508734096  };
...
void    poly_color(double t, t_rgb *color)
{
    ...
    for (i = 0; i < 3; ++i)
    {
        double v = MAG_C0[i] + t * (MAG_C1[i] + t * (MAG_C2[i] + t * (MAG_C3[i]
                 + t * (MAG_C4[i] + t * (MAG_C5[i] + t * MAG_C6[i])))));
        v = fmin(fmax(v * 255.0, 0.0), 255.0);
        rgb[i] = v;
    }
    color->r = (uint8_t)rgb[0];
    color->g = (uint8_t)rgb[1];
    color->b = (uint8_t)rgb[2];
}
