...
static inline int fast_julia(double zr, double zi, double cr, double ci, int max_iter)
{
	int i = 0;
	double zr2 = zr*zr, zi2 = zi*zi;
	while (zr2 + zi2 <= 4.0 && i < max_iter)
	{
		double tmp = 2.0 * zr * zi + ci;
		zr = zr2 - zi2 + cr;
		zi = tmp;
		zr2 = zr * zr; zi2 = zi * zi;
		++i;
...
static inline void store_rgba(mlx_image_t* img, int x, int y, uint32_t color)
{
	// color is 0xRRGGBBAA; place bytes in RGBA order in memory
	...
	while (y < y1)
	{
		...
		else if (d->fractal_type == FRACTAL_JULIA)
		{
			while (x < d->w)
			{
				...
				int iter = fast_julia(cr, ci, d->julia_c.r, d->julia_c.i, d->max_iter);
				uint32_t color = color_iteration(iter, d->max_iter, d->color_scheme, d);
				store_rgba(d->img, x, y, color);
				cr += d->dx; ++x;
			}
